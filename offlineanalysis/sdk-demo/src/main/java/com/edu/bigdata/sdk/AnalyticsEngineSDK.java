...
public class AnalyticsEngineSDK {
    ...
    public static boolean onChargeSuccess(String orderId,String memberId){
        return buildSendUrlData(orderId,memberId,"e_cs");
    }
    /**
     * 触发订单退款事件，发送退款数据到服务器
     *
    ...
    public static boolean onChargeRefund(String orderId,String memberId){
        return buildSendUrlData(orderId, memberId, "e_cr");
    }

    private static boolean buildSendUrlData(String orderId, String memberId, String en) {
        try {
    ...
    private static String buildUrl(Map<String,String> data)throws UnsupportedEncodingException {
        ...
        for (Map.Entry<String,String> entry:data.entrySet()){
            if (!StringUtils.isEmpty(entry.getKey())&&!StringUtils.isEmpty(entry.getKey())){
